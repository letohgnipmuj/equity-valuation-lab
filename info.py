...
def build_dcf_assumptions(
    ...
    if income is None or income.empty:
        raise ValueError("Income statement is empty; cannot build DCF assumptions.")

    if cashflow is None or cashflow.empty:
        raise ValueError("Cash flow statement is empty; cannot build DCF assumptions.")

    if balance is None or balance.empty:
        raise ValueError("Balance sheet is empty; cannot build DCF assumptions.")
    ...
    if pretax.empty or tax_provision.empty or pretax.iloc[0] == 0:
        raise ValueError("Cannot compute tax rate from provided income statement.")
    ...
    if "Cash And Cash Equivalents" not in balance.index:
        raise ValueError("Missing 'Cash And Cash Equivalents' in balance sheet.")
    ...
    short_term_debt = float(balance.loc["Current Debt"].iloc[0]) if "Current Debt" in balance.index else 0.0
    long_term_debt = float(balance.loc["Long Term Debt"].iloc[0]) if "Long Term Debt" in balance.index else 0.0
    ...
    if depreciation_series is None:
        raise ValueError("Could not locate depreciation in cash flow statement.")
